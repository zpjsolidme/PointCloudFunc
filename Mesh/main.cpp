...
void main()
{
	...
	// 设置各参数值
	gp3.setMu(30); //30设置被样本点搜索其近邻点的最远距离为2.5，为了使用点云密度的变化-----30
	gp3.setMaximumNearestNeighbors(10); //设置样本点可搜索的邻域个数----------100
	gp3.setMaximumSurfaceAngle(M_PI / 2); //设置某点法线方向偏离样本点法线的最大角度45-----4
	gp3.setMinimumAngle(M_PI / 18); // 设置三角化后得到的三角形内角的最小的角度为10---------18
	gp3.setMaximumAngle(2 * M_PI / 6); // 设置三角化后得到的三角形内角的最大角度为120--------2*pi*3
